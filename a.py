...
async def process_attack_ip_port(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="*Invalid port, time, threads, or payload_size.*",
            parse_mode='Markdown'
        )
    except Exception as e:
        print(f"Error in process_attack_ip_port: {e}")
        await context.bot.send_message(
            chat_id=chat_id,
            text="*Error processing attack parameters.*",
            parse_mode='Markdown'
        )
...
async def attack_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_attack_details[user_id] = True  # Flag to expect attack parameters
    await context.bot.send_message(
        chat_id=chat_id,
        text="*Please provide target IP, port, time (seconds), and optional threads and payload size (bytes) separated by spaces.*\nExample: 192.168.1.1 8080 120 [threads] [payload_size]",
        parse_mode='Markdown'
    )
