...
COST_PER_ATTACK = 1
RESTRICTED_PORTS = [17500, 20000, 20001, 20002]  # Fixed variable name
...
async def cleanup_attack(chat_id, context):
    ...
    try:
        if active_attack:
            ...
            # Terminate the attack process using pkill
            try:
                result = subprocess.run(
                    ['pkill', '-f', './MasterBhaiyaa'],
                    check=True,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE
                )
                print(f"pkill stdout: {result.stdout.decode()}")
                if result.stderr:
                    print(f"pkill stderr: {result.stderr.decode()}")
            except subprocess.CalledProcessError as e:
                print(f"Error terminating process with pkill: {e.stderr.decode()}")
            except Exception as e:
                print(f"Unexpected error during pkill: {e}")
        ...
        print(f"Error in cleanup_attack: {e}")
        await context.bot.send_message(
            chat_id=chat_id,
            text="❌ An error occurred during cleanup. Contact @MasterBhaiyaa",
            parse_mode='Markdown'
        )
    finally:
