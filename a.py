...
async def get_user_data(user_id):
    try:
        ...
        if user is None:
            print(f"No user found for user_id={user_id}, returning default")
            return {"user_id": user_id, "coins": 0}
        if "coins" not in user:
            print(f"User {user_id} found but missing 'coins' key, setting to 0")
            user["coins"] = 0
...
async def send_main_buttons(chat_id, context):
    try:
        # Define keyboard layout: two buttons per row
        keyboard = [
...
async def start_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f"Error in start_attack during coin deduction: {str(e)}")
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"*Error deducting coins: {str(e)}. Contact @MasterBhaiyaa*",
            parse_mode='Markdown'
        )
        return
...
async def admin_tools(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    action, user_id_str, amount_str = args[0], args[1], args[2]
    ...
    try:
        ...
        # Get user data
        user = await get_user_data(user_id)
        coins = user.get('coins', 0)  # Safely access 'coins' with default 0
        ...
        await update.message.reply_text(
            f"❌ Database error while updating coins: {str(e)}. Contact @MasterBhaiyaa",
            parse_mode='Markdown'
        )
    except Exception as e:
...
def main():
    try:
        ...
        mongo_client.server_info()  # Raises ConnectionFailure if connection fails
